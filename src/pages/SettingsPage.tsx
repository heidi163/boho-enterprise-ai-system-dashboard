...
export default function SettingsPage({ activeCompany }: { activeCompany: string }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full h-full pb-8" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel rounded-[24px] p-6 text-right">
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            <h2 className="text-xl font-black text-slate-800 font-tajawal tracking-tight">إعدادات النظام ({activeCompany === "bgk" ? "BGK" : "O2Nation"})</h2>
            <p className="text-xs text-gray-500 font-tajawal mt-1">إدارة الحساب، فريق العمل، الصلاحيات، والربط البرمجي (API)</p>
          </div>
        </div>
        
        <button className="bg-gradient-to-r from-slate-700 to-slate-900 text-white px-6 py-2.5 rounded-xl font-bold font-tajawal text-sm hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-md">
          حفظ التعديلات
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 flex-1">
        ...
        <div className="md:col-span-4 lg:col-span-3 flex flex-col gap-2">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all font-tajawal font-bold text-sm text-right flex-row-reverse justify-end ${
                activeTab === tab.id 
        ...
        {/* SETTINGS CONTENT AREA (Left Side) */}
        <div className="md:col-span-8 lg:col-span-9 glass-panel rounded-[24px] p-6 md:p-8 text-right">
          
          {activeTab === "profile" && (
            <div className="flex flex-col gap-6 animate-fade-in">
              <div className="border-b border-gray-100 pb-4 mb-2 text-right">
                <h3 className="font-bold text-lg text-slate-800 font-tajawal">الملف الشخصي لـ {activeCompany === "bgk" ? "Bohemian Geeks" : "O2Nation"}</h3>
              ...
              <div className="flex items-center justify-start gap-6 flex-row-reverse">
                <div className="w-20 h-20 rounded-full bg-gradient-to-bl from-blue-100 to-blue-200 border-4 border-white shadow-sm flex items-center justify-center text-blue-500 font-black text-2xl font-mono">
                  A
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                <div>
                  ...
                  <input type="text" defaultValue="أحمد صلاح" className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-tajawal text-slate-800 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 text-right" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 font-tajawal mb-1.5">البريد الإلكتروني</label>
                  <input type="email" defaultValue={activeCompany === "bgk" ? "ahmed@bohemiangeeks.com" : "ahmed@o2nation.com"} className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-tajawal text-slate-800 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 text-right" dir="ltr" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 font-tajawal mb-1.5">الشركة / الوكالة</label>
                  <input type="text" defaultValue={activeCompany === "bgk" ? "Bohemian Geeks" : "O2Nation"} readOnly className="w-full bg-white/40 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-tajawal text-slate-500 focus:outline-none text-right" />
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === "api" && (
            <div className="flex flex-col gap-6 animate-fade-in text-right">
              <div className="border-b border-gray-100 pb-4 mb-2">
                ...
                <p className="text-xs text-gray-500 font-tajawal mt-1">إدارة مفاتيح الـ API الخاصة بخدمات الذكاء الاصطناعي لـ {activeCompany === "bgk" ? "BGK" : "O2Nation"}</p>
              </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-3 flex-row-reverse">
                    ...
                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-mono font-bold" dir="ltr">Active</span>
                  </div>
                  <div className="flex gap-2 flex-row-reverse">
                    <input type="password" defaultValue="sk-ant-api03-xxxxxxxxxxxxxxxxxxxxxxxxx" readOnly className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm font-mono text-slate-600 focus:outline-none text-left" dir="ltr" />
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-3 flex-row-reverse">
                    ...
                    <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-mono font-bold" dir="ltr">Missing</span>
                  </div>
                  <div className="flex gap-2 flex-row-reverse">
                    <input type="text" value={elevenLabsKey} onChange={e => setElevenLabsKey(e.target.value)} placeholder="أدخل مفتاح ElevenLabs لتفعيل الصوت الاحترافي" className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm font-mono text-slate-800 focus:outline-none focus:border-blue-400 text-right" />
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-3 flex-row-reverse">
                    <label className="flex items-center justify-end gap-2 text-sm font-bold text-slate-800 font-tajawal w-full">X-Boho-Secret <Lock className="w-3.5 h-3.5 text-rose-500" /></label>
                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-mono font-bold" dir="ltr">Secure</span>
                  </div>
                  <div className="flex gap-2 flex-row-reverse">
                    <input type="password" defaultValue="task-server-secret-key-1234" readOnly className="flex-1 bg-rose-50 border border-rose-100 rounded-xl px-4 py-2 text-sm font-mono text-rose-600 focus:outline-none text-left" dir="ltr" />
            ...
            <div className="flex flex-col gap-6 animate-fade-in text-right">
              ...
              <div className="bg-white/60 border border-blue-100 rounded-2xl p-6">
                <div className="flex items-start justify-between mb-6 flex-row-reverse">
                  <div className="flex items-center justify-end gap-4 flex-row-reverse w-full">
                    <div className="w-12 h-12 bg-white shadow-sm border border-gray-100 rounded-xl flex items-center justify-center p-2 shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-mono text-gray-500 mt-0.5" dir="ltr">{activeCompany === "bgk" ? "ahmed@bohemiangeeks.com" : "ahmed@o2nation.com"}</p>
                    </div>
                  </div>
                  <span className="text-[10px] bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full font-tajawal font-bold border border-emerald-200 shrink-0">متصل (Connected)</span>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 flex-row-reverse">
                    <span className="text-xs font-bold text-slate-700 font-tajawal">صلاحية الجيميل (قراءة/إرسال)</span>
                    <input type="checkbox" defaultChecked className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                  </div>
                  <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 flex-row-reverse">
                    <span className="text-xs font-bold text-slate-700 font-tajawal">إدارة تقويم جوجل</span>
                    <input type="checkbox" defaultChecked className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                  </div>
                </div>

                <div className="flex items-center justify-start gap-3 mt-6 pt-4 border-t border-gray-100 flex-row-reverse">
              ...
              <div className="bg-white/60 border border-slate-200 rounded-2xl p-6">
                <div className="flex items-start justify-between mb-4 flex-row-reverse">
                  <div className="flex items-center justify-end gap-4 flex-row-reverse w-full">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-800 font-tajawal text-sm">{activeCompany === "bgk" ? "CRM (HubSpot)" : "CRM (Salesforce / ERP)"}</h4>
                      <p className="text-[11px] font-tajawal text-gray-500 mt-0.5">إدارة بيانات العملاء والمبيعات</p>
                    </div>
                  </div>
                  <span className="text-[10px] bg-slate-100 text-slate-500 px-3 py-1 rounded-full font-tajawal font-bold border border-slate-200 shrink-0">غير متصل</span>
                </div>
              ...
              <div className="bg-white/60 border border-slate-200 rounded-2xl p-6">
                <div className="flex items-start justify-between mb-4 flex-row-reverse">
                  <div className="flex items-center justify-end gap-4 flex-row-reverse w-full">
                    <div className="w-12 h-12 bg-[#25D366] bg-opacity-10 shadow-sm border border-[#25D366]/20 rounded-xl flex items-center justify-center p-2 shrink-0">
