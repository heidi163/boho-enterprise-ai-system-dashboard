...
export default function ProactiveCenterPage({ activeCompany }: { activeCompany: string }) {
  const defaultBriefings = activeCompany === "bgk" ? [
    { 
      id: 1,
      date: "اليوم 09:15", 
    ...
    { 
      id: 1,
      date: "اليوم 09:15", 
      text: "مرحباً أحمد، خدمات O2Nation بالأمس حققت 12 ألف جنيه، متجاوزة الهدف بـ 10%. الـ Leads لخدمات الـ ERP زادت بنسبة 20%. يوجد تنبيه: سيرفر استضافة العملاء استهلاكه تجاوز 90%. هل ترغب في زيادة السعة؟", 
      status: "sent", 
      isSmart: true,
      actionable: true,
      actionTaken: false
    ...
    { 
      id: 2,
      date: "أمس 09:00", 
      text: "صباح الخير أحمد. تقرير الأمس: تم إغلاق 3 عقود لتطوير السوفتوير بنجاح. أداء الإعلانات على LinkedIn ممتاز وتكلفة الليد انخفضت 15%. جميع مهام المتابعة تمت.", 
      status: "sent", 
      isSmart: false,
      actionable: false,
      actionTaken: false
  ...
  const defaultRules = activeCompany === "bgk" ? [
    { id: 1, label: "عائد الإعلانات (ROAS) تحت العتبة", threshold: "< 2.0", platform: "Windsor - كل الحسابات", active: true },
    { id: 2, label: "مبيعات يومية منخفضة", threshold: "< 30,000 EGP", platform: "Metorik - iFilter", active: true },
    { id: 3, label: "الإنفاق تجاوز الميزانية", threshold: "> 110%", platform: "Windsor - Meta", active: false },
  ] : [
    { id: 1, label: "استهلاك السيرفر مرتفع", threshold: "> 85%", platform: "AWS/DigitalOcean", active: true },
    { id: 2, label: "عقود مغلقة يومية منخفضة", threshold: "< 1", platform: "HubSpot", active: true },
    { id: 3, label: "الإنفاق تجاوز الميزانية", threshold: "> 110%", platform: "Windsor - LinkedIn", active: false },
  ];

  const defaultLogs = activeCompany === "bgk" ? [
    "> [16:00:12] Initializing Boho AI Proactive Engine for BGK...",
    "> [16:01:05] Scanning Meta Ads (Windsor.ai) for account: iFilter...",
    "> [16:01:08] ROAS threshold check passed (3.4 > 2.0)."
  ] : [
    "> [16:00:12] Initializing Boho AI Proactive Engine for O2Nation...",
    "> [16:01:05] Checking AWS server utilization...",
    "> [16:01:08] Server health optimal, CPU at 45%."
  ];

  const [briefingTime, setBriefingTime] = useState("09:00");
  const [roasCheckInterval, setRoasCheckInterval] = useState("3h");
  const [telegramConnected, setTelegramConnected] = useState(true);
  const [alertRules, setAlertRules] = useState(defaultRules);
  const [briefingHistory, setBriefingHistory] = useState(defaultBriefings);
  const [logs, setLogs] = useState<string[]>(defaultLogs);
  
  useEffect(() => {
    setAlertRules(defaultRules);
    setBriefingHistory(defaultBriefings);
    setLogs(defaultLogs);
  }, [activeCompany]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const possibleLogs = activeCompany === "bgk" ? [
        ...
        "> [SYSTEM] Fetching CRM contracts data...",
        "> [SYSTEM] Syncing server health metrics...",
        "> [ALERT_CHECK] Server load check initiated...",
        "> [ALERT_CHECK] All services operating within parameters.",
        "> [AI_ENGINE] Analyzing calendar for optimal briefing time..."
      ];
  ...
  const triggerBriefing = async () => {
    ...
    try {
      const res = await fetch(`/api/briefing?company_id=${activeCompany}`, { method: "POST" });
      const d = await res.json();
      setBriefingHistory(prev => [{ 
        id: Date.now(),
        date: "الآن " + new Date().toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" }), 
        text: d.briefing || (activeCompany === "bgk" ? "تم إرسال ملخص أداء المبيعات والإعلانات." : "تم إرسال ملخص أداء السيرفرات والخدمات."), 
        status: "sent",
        isSmart: false,
        actionable: false,
        actionTaken: false
      ...
      setBriefingHistory(prev => [{ 
        id: Date.now(),
        date: "الآن " + new Date().toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" }), 
        text: activeCompany === "bgk" ? "تم إرسال ملخص أداء المبيعات والإعلانات." : "تم إرسال ملخص أداء السيرفرات والخدمات.", 
        status: "sent",
        isSmart: false,
        actionable: false,
        actionTaken: false
  ...
  const triggerRoasCheck = async () => {
    ...
    try {
      await fetch(`/api/alert/roas?company_id=${activeCompany}`, { method: "POST" });
      setLogs(prev => [...prev.slice(-4), `> [${new Date().toLocaleTimeString("en-GB")}] MANUAL SYSTEM SCAN INITIATED...`]);
    } catch {
      setLogs(prev => [...prev.slice(-4), `> [${new Date().toLocaleTimeString("en-GB")}] MANUAL SYSTEM SCAN INITIATED...`]);
    }
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="glass-panel rounded-full px-5 py-2.5 flex items-center gap-6 w-full md:w-auto shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono text-slate-600 font-bold flex items-center gap-1.5" dir="ltr"><Send className="w-3 h-3 text-blue-500" /> @ahmed_bgk_bot</span>
          </div>
          <div className="w-px h-4 bg-slate-200" />
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[10px] font-mono text-slate-600 font-bold flex items-center gap-1.5" dir="ltr"><Activity className="w-3 h-3 text-emerald-500" /> n8n Engine</span>
          </div>
          <div className="w-px h-4 bg-slate-200 hidden md:block" />
          <div className="hidden md:flex items-center gap-2">
            <span className="text-[10px] font-mono text-slate-500 font-bold flex items-center gap-1.5" dir="ltr"><Clock className="w-3 h-3 text-slate-400" /> Cron: {n8nStatus.lastRun}</span>
          </div>
        </div>

        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          <button onClick={triggerRoasCheck} disabled={manualRoasLoading} className="flex items-center gap-1.5 text-xs text-white bg-gradient-to-r from-rose-500 to-red-600 rounded-full px-4 py-2 font-bold hover:opacity-90 disabled:opacity-50 transition-opacity shadow-sm">
            <Zap className="w-3.5 h-3.5" /> {manualRoasLoading ? "جارٍ الفحص..." : "فحص الأداء الآن"}
          </button>
          <button onClick={triggerBriefing} disabled={manualBriefLoading} className="flex items-center gap-1.5 text-xs text-white bg-gradient-to-r from-blue-500 to-blue-600 rounded-full px-4 py-2 font-bold hover:opacity-90 disabled:opacity-50 transition-opacity shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-5 flex flex-col gap-5">
          ...
          <div className={`glass-panel rounded-[24px] p-5 transition-all duration-500 border-t-4 ${isSmartTiming ? "border-t-sky-400" : "border-t-slate-300"}`}>
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">الذكاء الاصطناعي</span>
            ...
            <div className="flex items-center justify-between bg-white/60 rounded-xl p-3 border border-white/80 mb-4 shadow-sm text-right">
            ...
            {isSmartTiming ? (
              <div className="bg-white/40 border border-sky-100 rounded-xl p-4 flex flex-col gap-3">
                <div className="flex items-center justify-between flex-row-reverse">
                  ...
                  <span className="text-xl font-black font-mono text-sky-700" dir="ltr">09:15 AM</span>
                </div>
                <div className="text-[11px] text-slate-700 font-tajawal leading-relaxed bg-white/50 p-2 rounded-lg border border-white/60 text-right">
              ...
              <div className="bg-white/40 border border-slate-100 rounded-xl p-4 flex items-center justify-between flex-row-reverse">
                <span className="text-xs text-slate-600 font-tajawal font-bold">وقت ثابت يومياً</span>
                <input type="time" value={briefingTime} onChange={e => setBriefingTime(e.target.value)} className="bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-mono text-slate-700 outline-none focus:border-blue-300" dir="ltr" />
              </div>
            )}
          </div>
          ...
          <div className="glass-panel rounded-[24px] p-5">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">قواعد التنبيه</span>
            ...
            <div className="flex items-center justify-between bg-white/50 rounded-xl px-4 py-3 border border-white/80 mb-3 flex-row-reverse">
              <span className="text-[10px] font-tajawal font-bold text-slate-700">دورية الفحص (Cron)</span>
              <select value={roasCheckInterval} onChange={e => setRoasCheckInterval(e.target.value)} className="bg-transparent text-[10px] font-mono font-bold text-blue-600 outline-none cursor-pointer text-right">
            ...
            <div className="flex flex-col gap-2">
              {alertRules.map(r => (
                <div key={r.id} className={`rounded-xl px-4 py-3 border flex items-center justify-between transition-all flex-row-reverse ${r.active ? "bg-white/60 border-white/80" : "bg-slate-50 border-slate-200 opacity-60"}`}>
                  <div className="flex items-center justify-end gap-3 w-full">
                    <div className="text-right flex items-center gap-2 justify-end w-full">
                      <p className="text-xs font-bold text-slate-700 font-tajawal">{r.label}</p>
                      <span className="text-[10px] font-mono text-rose-500 font-bold bg-rose-50 px-1.5 py-0.5 rounded" dir="ltr">{r.threshold}</span>
                    </div>
                    <span className="text-[9px] font-mono text-gray-400 bg-white/50 px-1.5 py-0.5 rounded shrink-0" dir="ltr">{r.platform}</span>
                    <button onClick={() => toggleRule(r.id)} className={`w-9 h-5 rounded-full transition-colors relative shrink-0 ${r.active ? "bg-emerald-500" : "bg-slate-300"}`}>
        ...
        <div className="lg:col-span-7 flex flex-col gap-5">
          ...
          <div className="glass-panel rounded-[24px] p-5 bg-slate-900 border-none relative overflow-hidden group">
            <div className="absolute top-0 left-0 w-32 h-32 bg-blue-500/20 rounded-full blur-3xl -ml-10 -mt-10 pointer-events-none" />
            <div className="flex items-center justify-between mb-3 relative z-10 border-b border-slate-700 pb-2 flex-row-reverse">
              <div className="flex gap-1.5 flex-row-reverse">
                <div className="w-2.5 h-2.5 rounded-full bg-rose-500" />
                <div className="w-2.5 h-2.5 rounded-full bg-amber-500" />
                <div className="w-2.5 h-2.5 rounded-full bg-emerald-500" />
              </div>
              <h3 className="text-[11px] font-mono text-slate-400 flex items-center gap-1.5" dir="ltr"><Terminal className="w-3.5 h-3.5" /> BOHO_ENGINE_LIVE</h3>
            </div>
            
            <div className="h-[100px] w-full overflow-hidden flex flex-col justify-end font-mono text-[10px] leading-relaxed relative z-10 text-left" dir="ltr">
               {logs.map((log, i) => (
          ...
          <div className="glass-panel rounded-[24px] p-5 flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">تيليجرام</span>
            ...
            <div className="flex-1 overflow-y-auto sidebar-scroll pr-1 flex flex-col gap-4">
              {briefingHistory.map((b) => (
                <div key={b.id} className="border border-white/80 rounded-2xl p-4 bg-white/50 shadow-sm relative overflow-hidden transition-all hover:shadow-md">
                  {b.isSmart && <div className="absolute top-0 left-0 w-8 h-8 bg-sky-500/10 rounded-br-full flex items-start justify-start p-1.5"><Sparkles className="w-3 h-3 text-sky-500" /></div>}
                  <div className="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono text-gray-400" dir="ltr">{b.date}</span>
                    </div>
                  </div>
                  <p className="text-sm text-slate-700 font-tajawal leading-relaxed text-right mt-1">{b.text}</p>
                  
                  {b.actionable && !b.actionTaken && (
                    <div className="mt-4 flex items-center justify-start gap-2 bg-slate-50 p-2 rounded-xl border border-slate-200">
                  ...
                  {b.actionTaken && (
                    <div className="mt-4 flex items-center justify-start gap-1.5 text-xs font-tajawal font-bold text-emerald-600 bg-emerald-50 p-2 rounded-xl border border-emerald-100">
