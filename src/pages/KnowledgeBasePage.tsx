...
export default function KnowledgeBasePage() {
  ...
  const documents = [
    { id: 1, title: "سياسة الإجازات 2026", type: "PDF", category: "HR Policies", date: "اليوم", stars: true },
    { id: 2, title: "عقد وكالة Sealy السنوي", type: "Contract", category: "Legal", date: "منذ يومين", stars: true },
    { id: 3, title: "SOP - إطلاق حملات ميتا", type: "Docs", category: "Marketing SOPs", date: "الأسبوع الماضي", stars: false },
    { id: 4, title: "تقرير الربع الأول 2026", type: "Excel", category: "Finance", date: "الشهر الماضي", stars: false },
  ];
  ...
  return (
    <div className="flex flex-col gap-6 w-full" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel rounded-[24px] p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-tajawal mt-1">Semantic Search وذاكرة الشركة (Documents, SOPs, Contracts)</p>
          </div>
        </div>
        ...
        {[
          ...
          <div key={idx} className="glass-panel rounded-[20px] p-4 flex items-center justify-between cursor-pointer hover:scale-[1.02] transition-transform">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-slate-700 text-sm font-tajawal">{cat.label}</span>
            </div>
      ...
      <div className="glass-panel rounded-[24px] p-6 flex-1">
        <h3 className="text-sm font-bold text-slate-800 font-tajawal mb-4 border-b border-gray-100 pb-3">المستندات الأخيرة</h3>
        
        <div className="flex flex-col gap-3">
          {documents.map((doc) => (
            <div key={doc.id} className="flex items-center justify-between p-3 hover:bg-white/50 rounded-xl transition-colors border border-transparent hover:border-gray-200 cursor-pointer group">
