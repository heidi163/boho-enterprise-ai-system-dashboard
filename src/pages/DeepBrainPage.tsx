...
export default function DeepBrainPage({ activeCompany }: { activeCompany: string }) {
  ...
  const fetchMemory = async () => {
    ...
    try {
      ...
      console.error(e);
      // Fallback based on activeCompany
      if (activeCompany === "o2nation") {
        setMemory([
          { key: "الشركة", value: "O2Nation" },
          { key: "قاعدة", value: "التركيز على مبيعات السوفتوير" }
        ]);
      } else {
        setMemory([
          { key: "الشركة", value: "Bohemian Geeks" },
          { key: "قاعدة", value: "لا يوجد خصم أكبر من 10%" }
        ]);
      }
    }
  ...
  useEffect(() => { fetchMemory(); }, [activeCompany]);
  useEffect(() => {
  ...
  const handleSend = async (customText?: string) => {
    ...
    if (textToSend.includes("مبيعات") || textToSend.includes("iFilter")) {
      setTimeout(() => setMcpStatus("جاري الاستعلام من Metorik..."), 500);
      ...
      setTimeout(() => setMcpStatus("بيفكر باستخدام Claude..."), 500);
    }
    ...
    try {
      ...
      setMcpStatus(null);
      typeMessage(data.content || "تمت المعالجة بنجاح.");
      removeImage(); 
    } catch {
      setMcpStatus(null);
      typeMessage("عذراً، فيه مشكلة في الاتصال بالعقل العميق.");
    }
    setLoading(false);
    fetchMemory();
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full h-full pb-8" dir="rtl">
      ...
      {/* ARCHITECTURE HEADER */}
      <div className="glass-panel rounded-[20px] p-4 flex flex-wrap gap-4 items-center justify-between border-blue-200/50 bg-gradient-to-l from-blue-50/50 to-transparent">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-gray-200 shadow-sm text-[10px] font-mono text-slate-600 font-bold">
            ...
            طبقة الصوت: شغالة (~320ms)
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-gray-200 shadow-sm text-[10px] font-mono text-slate-600 font-bold">
            <div className="w-2 h-2 rounded-full bg-blue-500" />
            Claude {modelTier === "sonnet" ? "3.5 Sonnet" : "3 Opus"}
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-gray-200 shadow-sm text-[10px] font-mono text-slate-600 font-bold">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 flex-1 min-h-[600px]">
        ...
        <div className="xl:col-span-3 flex flex-col gap-5 h-[700px]">
          <div className="glass-panel rounded-[24px] p-5 flex flex-col h-full">
            ...
            <div className="relative mb-5">
              ...
              <input type="text" placeholder="ابحث في السجل..." className="w-full bg-white/60 border border-gray-200 rounded-xl py-2 pl-9 pr-3 text-xs font-tajawal outline-none focus:border-blue-400 transition-colors" />
            </div>

            <div className="flex-1 overflow-y-auto sidebar-scroll pl-1 flex flex-col gap-4">
              
              {/* Quick Prompts */}
              <div>
                <div className="text-[10px] font-bold text-blue-500 mb-2 font-tajawal uppercase tracking-wide">أوامر سريعة</div>
                <div className="flex flex-col gap-2">
                  <button onClick={() => handleSend(activeCompany === "bgk" ? "إجمالي مبيعات iFilter امبارح كام؟" : "إجمالي مبيعات O2Nation الشهر ده كام؟")} className="text-right text-[11px] font-tajawal p-2 rounded-lg bg-blue-50/50 hover:bg-blue-100/50 border border-blue-100 text-blue-700 transition-colors">
                    إجمالي المبيعات امبارح؟
                  </button>
              ...
              <div>
                <div className="text-[10px] font-bold text-slate-400 mb-2 font-tajawal">المفضلة</div>
                <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-2.5 cursor-pointer hover:bg-amber-50 transition-colors flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0 text-right">
                    <p className="text-xs font-bold text-slate-700 font-tajawal truncate">تحليل مبيعات {activeCompany === "bgk" ? "رمضان" : "الربع الأخير"}</p>
              ...
              <div>
                ...
                {[
                  activeCompany === "bgk" ? "مراجعة إعلانات Sealy" : "مراجعة إعلانات B2B",
                  activeCompany === "bgk" ? "صياغة إيميل لشركة O2Nation" : "صياغة إيميل للعميل",
        ...
        <div className="xl:col-span-5 glass-panel rounded-[24px] p-5 flex flex-col h-[700px] relative">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-white bg-blue-500 px-2 py-0.5 rounded-full font-bold">
                {modelTier === "sonnet" ? "الموديل السريع" : "الموديل العميق"}
              </span>
              <span className="text-[10px] font-mono text-gray-400">Claude {modelTier === "sonnet" ? "3.5 Sonnet" : "3 Opus"} + أدوات MCP</span>
            </div>
          ...
          <div ref={chatRef} className="flex-1 overflow-y-auto sidebar-scroll pl-2 flex flex-col gap-5 pb-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
                <div className={`max-w-[85%] rounded-2xl px-5 py-3.5 text-sm font-tajawal leading-relaxed shadow-sm text-right ${
                  m.role === "user" ? "bg-white/80 text-slate-800 border border-gray-100" : "bg-gradient-to-bl from-blue-500 to-blue-600 text-white"
                }`}>
                  {m.content}
                  {m.isStreaming && <span className="inline-block w-1.5 h-4 ms-1 bg-white animate-pulse align-middle" />}
                </div>
              </div>
            ))}
            ...
            {loading && (
              <div className="flex justify-end flex-col items-end gap-2 text-right">
                {mcpStatus && (
                ...
                <div className="bg-gradient-to-bl from-blue-500 to-blue-600 rounded-2xl px-5 py-3.5 text-white flex gap-1.5 shadow-sm w-fit">
          ...
          <div className="relative mt-2 flex flex-col gap-2">
            {imagePreview && (
              <div className="relative inline-block self-end">
                ...
                <button onClick={removeImage} className="absolute -top-2 -left-2 bg-rose-500 text-white rounded-full p-1 hover:bg-rose-600 shadow-md">
                  <X className="w-3 h-3" />
                </button>
              </div>
            )}
            <div className="relative">
              <input
                ...
                className="w-full bg-white/60 border border-gray-200 rounded-2xl px-5 py-4 pl-14 pr-14 text-sm font-tajawal text-slate-800 outline-none shadow-sm placeholder-gray-400 focus:bg-white focus:border-blue-400 transition-colors"
              />
              <button
                onClick={() => handleSend()}
                disabled={(!input.trim() && !imageBase64) || loading}
                className="absolute left-2 top-2 bottom-2 w-10 bg-blue-500 text-white rounded-xl flex items-center justify-center hover:bg-blue-600 disabled:opacity-50 transition-colors"
              >
              ...
              <button
                ...
                className="absolute right-2 top-2 bottom-2 w-10 bg-slate-100 text-slate-500 rounded-xl flex items-center justify-center hover:bg-slate-200 transition-colors"
                title="إرفاق صورة"
              >
        ...
        <div className="xl:col-span-4 flex flex-col gap-5 h-[700px] overflow-y-auto sidebar-scroll pl-1 pr-2 pb-2">
          
          {/* TOKEN METER */}
          <div className="glass-panel rounded-[24px] p-5 shrink-0">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">الاستهلاك والتكلفة</span>
            ...
            <div className="flex flex-col gap-4">
              <div>
                <div className="flex justify-between items-end mb-1.5 text-right">
                ...
                {tokenPercentage >= 80 && <p className="text-[9px] text-rose-500 mt-1 font-bold text-right">تحذير: اقتربت من الحد الأقصى للميزانية</p>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/50 border border-white/80 rounded-xl p-3 flex flex-col items-center justify-center relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-8 h-8 bg-emerald-500/10 rounded-br-full flex items-start justify-start p-1.5"><DollarSign className="w-3 h-3 text-emerald-600" /></div>
          ...
          <div className="glass-panel rounded-[24px] p-5 shrink-0">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">n8n مهام مجدولة</span>
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => triggerProactive("/briefing")} className="text-right flex items-center justify-between p-3 rounded-xl bg-white/50 border border-gray-100 hover:border-emerald-200 transition-colors group">
                <span className="text-[10px] font-mono text-gray-400 group-hover:text-emerald-500 whitespace-nowrap ms-2">POST /briefing</span>
              ...
              <button onClick={() => triggerProactive("/alert/roas")} className="text-right flex items-center justify-between p-3 rounded-xl bg-white/50 border border-gray-100 hover:border-emerald-200 transition-colors group">
                <span className="text-[10px] font-mono text-gray-400 group-hover:text-emerald-500 whitespace-nowrap ms-2">POST /alert/roas</span>
                <span className="text-xs font-bold text-slate-700 font-tajawal">فحص الإعلانات (ROAS)</span>
              </button>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-[24px] p-5 shrink-0">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">أدوات حية</span>
            ...
            <div className="flex flex-col gap-2.5">
              {[
                { name: "Metorik MCP", sub: activeCompany === "bgk" ? "iFilter, Sealy" : "O2Nation Stores", status: "online", color: "bg-emerald-500" },
                { name: "Windsor MCP", sub: "Meta, Google", status: "online", color: "bg-emerald-500" },
                { name: "WordPress MCP", sub: "غير مفعل", status: "warning", color: "bg-amber-500" },
                { name: "Shopify MCP", sub: "غير مفعل", status: "offline", color: "bg-gray-300" },
              ].map((s, i) => (
          ...
          <div className="glass-panel rounded-[24px] p-5 flex flex-col min-h-[250px]">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase">قاعدة البيانات</span>
            ...
            <div className="flex items-center gap-2 mb-3">
              <input type="text" value={newFactValue} onChange={(e) => setNewFactValue(e.target.value)} placeholder="القيمة..." className="w-1/2 bg-white/60 border border-gray-200 rounded-lg px-2 py-1 text-[10px] font-mono outline-none text-right" />
              <input type="text" value={newFactKey} onChange={(e) => setNewFactKey(e.target.value)} placeholder="المفتاح..." className="w-1/2 bg-white/60 border border-gray-200 rounded-lg px-2 py-1 text-[10px] font-mono outline-none text-right" />
            ...
            <div className="flex-1 overflow-y-auto sidebar-scroll pl-1 flex flex-col gap-2">
              {memoryLoading ? (
                ...
                memory.map((f, i) => (
                  <div key={i} className="flex items-center justify-between bg-white/60 border border-gray-100 rounded-xl px-3 py-2 group">
                    ...
                    <div className="flex items-center justify-between flex-1 pr-2">
                      <span className="text-[10px] font-mono text-blue-600 font-bold text-left truncate w-[80px]" dir="ltr">{f.value}</span>
